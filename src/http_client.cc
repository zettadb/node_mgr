...
#define HTTP_GET_RANGE_STR                                                     \
  "GET /%s HTTP/1.0\r\n\
HOST: %s:%d\r\n\
Accept: */*\r\n\
Range: bytes=%s\r\n\r\n"

#define HTTP_POST_PARA_STR                                                     \
  "POST /%s HTTP/1.0\r\n\
HOST: %s:%d\r\n\
Accept: */*\r\n\
...
bool Http_client::Http_client_parse_url(const char *url, char *ip, int *port,
                                        char *path) {
  char *cStart, *cEnd;
  int len = 0;
  if (!url || !ip || !port || !path)
...
bool Http_client::Http_client_get_filename(const char *filepath,
 ...
 */
int Http_client::Http_client_get_file(const char *url, const char *filename,
                                      int *pos) {
  char ip[100] = {'\0'};
  char path[256] = {'\0'};
  int port;
 ...
 *  -6 : file length download error
 */
int Http_client::Http_client_get_file_range(const char *url,
                                            const char *filename, int *pos,
                                            int begin, int end) {
  char ip[100] = {'\0'};
  char path[256] = {'\0'};
  int port;
  ...
  *pos = begin;
  ...
  int n = snprintf(http_buf, BUFSIZE, HTTP_GET_RANGE_STR, path, ip, port,
                   str.c_str());
  if (n >= sizeof(http_buf) - 1) {
    syslog(Logger::ERROR, "path %s is to long", path);
    return -2;
 ...
 *  -4 : http return error
 */
int Http_client::Http_client_post_para(const char *url, const char *post_str,
  ...
  int n = snprintf(http_buf, BUFSIZE, HTTP_POST_PARA_STR, path, ip, port,
 ...
 *  -6 : http return error
 */
int Http_client::Http_client_post_file(const char *url, const char *post_str,
                                       const char *filepath,
                                       std::string &result_str) {
  // syslog(Logger::INFO, "Http_client_post_file: %s,%s,%s,%s", url, post_str,
  // filename, filepath);
  ...
  //////////////////////////////////////////////////////////////////////
  // get content len
  contentlen = snprintf(http_buf, BUFSIZE, HTTP_POST_FILE_STR2, post_str,
                        filename.c_str());
  contentlen += filelen;
  ...
  int n = snprintf(http_buf, sizeof(http_buf), HTTP_POST_FILE_STR1, path, ip,
                   port, contentlen);
  n += snprintf(http_buf + n, sizeof(http_buf), HTTP_POST_FILE_STR2, post_str,
                filename.c_str());
  if (n >= sizeof(http_buf) - 1) {
    syslog(Logger::ERROR, "post_str %s is to long", post_str);
    close(socket_fd);
