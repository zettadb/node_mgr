...
bool System::regiest_to_meta()
{
	...
	kunlun::MysqlConnection *mysql_conn = new kunlun::MysqlConnection(options);
	if (!mysql_conn->Connect())
	{
		syslog(Logger::ERROR, "connect to metadata db failed: %s", mysql_conn->getErr());
		return false;
	}
	char addr[256] = {0};
	if (kunlun::GetIpFromInterface(interface_name.c_str(), addr) != 0)
	{
		return false;
	}

	local_ip = addr;
	syslog(Logger::INFO, "Get addr string '%s' from interface: %s", local_ip.c_str(), interface_name.c_str());

	kunlun::MysqlResult result_set;
	char sql[2048] = {0};
	sprintf(sql, "select * from kunlun_metadata_db.server_nodes where hostaddr = '%s'", local_ip.c_str());

	int ret = mysql_conn->ExcuteQuery(sql, &result_set);
	if (ret != 0)
	{
		syslog(Logger::ERROR, "metadata db query:[%s] failed: %s", sql, mysql_conn->getErr());
		return false;
	}
	if (result_set.GetResultLinesNum() > 0)
	{
		...
		for (int i = 1; i <= lines; i++)
		{
			...
			sprintf(sql, "delete from kunlun_metadata_db.server_nodes where id = %d limit 1;", result_set[i]["id"]);
			kunlun::MysqlResult rs;
			ret = mysql_conn->ExcuteQuery(sql, &rs);
			if (ret <= 0)
			{
				syslog(Logger::ERROR, "query:[%s] should affect at least one rows", sql);
			}
		}
	}
	bzero(sql, sizeof(sql) / sizeof(sql[0]));
	sprintf(sql, "insert into kunlun_metadata_db.server_nodes set hostaddr='%s',nodemgr_port=%d,total_cpu_cores=8, total_mem=16384,svc_since=current_timestamp,",
					local_ip.c_str(), node_mgr_http_port);
	ret = mysql_conn->ExcuteQuery(sql, &result_set);
	if (ret <= 0)
	{
		syslog(Logger::ERROR, "regiest current nodemanager to metadata db failed: %s", mysql_conn->getErr());
		return false;
	}
	return true;
}
