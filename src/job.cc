...
int Job::start_job_thread() {
  ...
  for (int i = 0; i < num_job_threads; i++) {
    ...
    if ((error = pthread_create(&hdl, NULL, thread_func_job_work, m_inst))) {
      char errmsg_buf[256];
      syslog(Logger::ERROR,
             "Can not create http server work thread, error: %d, %s", error,
...
bool Job::get_node_info(cJSON *root, std::string &str_ret) {
  ...
  cJSON_AddNumberToObject(
      ret_root, "meta_node",
      Instance_info::get_instance()->vec_meta_instance.size());
  cJSON_AddNumberToObject(
      ret_root, "storage_node",
      Instance_info::get_instance()->vec_storage_instance.size());
  cJSON_AddNumberToObject(
      ret_root, "computer_node",
      Instance_info::get_instance()->vec_computer_instance.size());
...
bool Job::start_node_exporter(cJSON *root, std::string &str_ret) {
  ...
  cmd = "netstat -tnpl | grep tcp6 | grep " +
        std::to_string(prometheus_port_start + 1);
  ...
  if (process_id.length() == 0) {
    ...
    cmd += "./node_exporter --web.listen-address=:" +
           std::to_string(prometheus_port_start + 1) + " &";
...
bool Job::get_path_size(std::string &path, std::string &used,
                        std::string &free) {
  bool ret = false;
  FILE *pfd = NULL;
...
bool Job::check_timestamp() {
  ...
  int ret = Http_client::get_instance()->Http_client_post_para(
      post_url.c_str(), cjson, result_str);
  free(cjson);
...
void Job::get_local_ip() {
  ...
  for (int i = 0; i < num; i++) {
    char *tmp_ip =
        inet_ntoa(((struct sockaddr_in *)(&ifq[i].ifr_addr))->sin_addr);
...
bool Job::get_datatime(std::string &datatime) {
  ...
  snprintf(sysTime, 128, "%04u-%02u-%02u %02u:%02u:%02u", tm->tm_year + 1900,
...
bool Job::get_table_path(std::string &ip, int port, std::string &user,
                         std::string &psw, std::string &db, std::string &tb,
                         std::string &tb_path) {
  syslog(Logger::INFO,
         "get_table_path ip=%s,port=%d,user=%s,psw=%s,db=%s,tb=%s", ip.c_str(),
         port, user.c_str(), psw.c_str(), db.c_str(), tb.c_str());

  int retry = stmt_retries;
  MYSQL_CONN mysql_conn;

  while (retry--) {
    if (mysql_conn.connect(db.c_str(), ip.c_str(), port, user.c_str(),
                           psw.c_str())) {
      syslog(Logger::ERROR,
             "connect to mysql error ip=%s,port=%d,user=%s,psw=%s,db=%s",
  ...
  tb_path = tb_path + db.substr(0, pos1) + "_@0024@0024_" +
            db.substr(pos1 + 4, db.length() - pos1 + 4);
  tb_path = tb_path + "/" + tb + ".ibd";

  return true;
}

bool Job::get_binlog_path(std::string &ip, int port, std::string &user,
                          std::string &psw, std::string &binlog_path) {
...
bool Job::get_cnf_path(std::string &ip, int port, std::string &user,
                       std::string &psw, std::string &cnf_path) {
  syslog(Logger::INFO, "get_cnf_path ip=%s,port=%d,user=%s,psw=%s", ip.c_str(),
         port, user.c_str(), psw.c_str());
...
bool Job::update_variables(std::string &ip, int port, std::string &user,
                           std::string &psw, std::string &name,
                           std::string &value) {
  syslog(Logger::INFO,
         "update_variables ip=%s,port=%d,user=%s,psw=%s,name=%s,value=%s",
         ip.c_str(), port, user.c_str(), psw.c_str(), name.c_str(),
         value.c_str());
...
bool Job::delete_db_table(std::string &ip, int port, std::string &user,
                          std::string &psw, std::string &db, std::string &tb) {
  syslog(Logger::INFO,
         "delete_db_table ip=%s,port=%d,user=%s,psw=%s,db=%s,tb=%s", ip.c_str(),
         port, user.c_str(), psw.c_str(), db.c_str(), tb.c_str());
...
void Job::job_send(cJSON *root) {
  ...
  if (file_type == FILE_TABLE) {
    ...
    std::string post_url =
        "http://" + remote_ip + ":" + std::to_string(node_mgr_http_port);
    ...
    std::string url = "http://" + ip + ":" +
                      std::to_string(node_mgr_http_port) + "/" + job_id;
    ...
    std::string result_str;
    Http_client::get_instance()->Http_client_post_para(post_url.c_str(), cjson,
                                                       result_str);
    free(cjson);
  } else if (file_type == FILE_BINLOG) {
  }
...
void Job::job_recv(cJSON *root) {
  ...
  if (file_type == FILE_TABLE) {
    ...
    int pos = 0;
    if (Http_client::get_instance()->Http_client_get_file(
            http_url.c_str(), tb_path.c_str(), &pos) != 0) {
      int retry = 6;
      while (retry > 0 &&
             Http_client::get_instance()->Http_client_get_file_range(
                 http_url.c_str(), tb_path.c_str(), &pos, pos, 0) != 0)
        retry--;
    }
  } else if (file_type == FILE_BINLOG) {
  }
...
bool Job::update_jobid_status(std::string &jobid, std::string &result,
                              std::string &info) {
  ...
  bool is_exist = false;
  for (auto it = list_jobid_result_info.begin();
       it != list_jobid_result_info.end(); ++it) {
...
bool Job::get_jobid_status(std::string &jobid, std::string &result,
                           std::string &info) {
  std::lock_guard<std::mutex> lock(mutex_stauts_);
...
bool Job::job_handle_ahead(const std::string &para, std::string &str_ret) {
  ...
  if (item == NULL ||
      !Job::get_instance()->get_job_type(item->valuestring, job_type)) {
