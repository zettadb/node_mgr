...
void Http_server::Http_server_handle_post_para(int &socket, char* buf, int len)
{
	...
	if(ret)
	{
		int n = snprintf(buf, BUFSIZE, lpHttpHtmlOk, str_ret.length(), str_ret.c_str());
		send(socket, buf, n, 0);
	}
	else
	{
		...
		int n = snprintf(buf, BUFSIZE, lpHttpHtmlOk, strlen(lpReturnOk), lpReturnOk);
		send(socket, buf, n, 0);
	}
}
...
void Http_server::Http_server_accept()
{
	...
	if(listenfd>0)
		syslog(Logger::INFO, "Http server listen at port: %d start", node_mgr_http_port);
	else
		syslog(Logger::ERROR, "Http server listen at port: %d fail", node_mgr_http_port);
