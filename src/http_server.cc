...
static const char *lpHttpHtmlOk = "HTTP/1.1 200 OK\r\nContent-Type: "
                                  "text/html\r\nContent-Length: %ld\r\n\r\n%s";
static const char *lpHttpHtmlOkTime =
...
static const char *lpHttpBinOk =
    "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nContent-Length: ";
static const char *lpHttpJspOk =
    "HTTP/1.1 200 OK\r\napplication/javascript\r\nContent-Length: ";
static const char *lpHttpJsonOk =
    "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: ";
static const char *lpHttpError = "HTTP/1.1 400 Bad Request\r\n";
static const char *lpReturnOk = "{\"result\":\"accept\"}";

static const char *lpHttpOPTIONS =
    "HTTP/1.1 200 OK\r\nContent-Type: application/json; charset=utf-8\r\n\
Access-Control-Max-Age: 3600\r\n\
Access-Control-Allow-Origin: *\r\n\
Access-Control-Allow-Headers: *\r\n\
...
Http_server::Http_server() : wakeupfd(0) {}
...
int Http_server::start_http_thread() {
  ...
  pthread_t hdl;
  if ((error = pthread_create(&hdl, NULL, thread_func_http_server_accept,
                              m_inst))) {
    char errmsg_buf[256];
    syslog(Logger::ERROR,
           "Can not create http server accept thread, error: %d, %s", error,
  ...
  for (int i = 0; i < num_http_threads; i++) {
    pthread_t hdl;
    if ((error = pthread_create(&hdl, NULL, thread_func_http_server_work,
                                m_inst))) {
      char errmsg_buf[256];
      syslog(Logger::ERROR,
...
bool Http_server::Get_http_range(const char *buf, uint64_t *begin,
                                 uint64_t *end) {
  char *cStart, *cEnd;
  std::string str;
...
int Http_server::Listen_socket(int port) {
  ...
  if (setsockopt(listenfd, SOL_SOCKET, SO_SNDTIMEO, &timeout,
                 sizeof(struct timeval))) {
  ...
  if (ret <= 0) {
    ...
    if (type == HTTP_GET) {
      ...
      if (content_type == Application_json ||
          content_type == Content_form_urlencoded)
...
void Http_server::Http_server_handle_get(int &socket, std::string &path) {
  ...
  if (path.length() == 0 || path == "hello") {
    ...
    if (!Job::get_instance()->get_file_path(path, strpath))
...
void Http_server::Http_server_handle_get_range(int &socket, std::string &path,
  ...
  if (res == 0) {
    ...
    uint64_t n = snprintf(http_buf, BUFSIZE, lpHttpRangeOk, begin, end, len,
                          need_to_send);
...
void Http_server::Http_server_handle_post_para(int &socket, char *buf,
                                               int len) {
  char *cStart, *cEnd;
  std::string para, str;
  int content_length;
  ...
  if (ret) {
    int n =
        snprintf(buf, BUFSIZE, lpHttpHtmlOk, str_ret.length(), str_ret.c_str());
    send(socket, buf, n, 0);
  } else {
    Job::get_instance()->add_job(para);

    int n =
        snprintf(buf, BUFSIZE, lpHttpHtmlOk, strlen(lpReturnOk), lpReturnOk);
    send(socket, buf, n, 0);
  }
}

void Http_server::Http_server_handle_post_file(int &socket, char *buf,
                                               int len) {
  ...
  file_length =
      content_length - (fileStart - contentStart) - boundary.length() - 8;
...
void Http_server::Http_server_accept() {
  ...
  int listenfd = Listen_socket(node_mgr_http_port);
  if (listenfd > 0)
    syslog(Logger::INFO, "Http server listen at port: %d start",
           node_mgr_http_port);
  else
    syslog(Logger::ERROR, "Http server listen at port: %d fail",
           node_mgr_http_port);
  ...
  while (!Http_server::do_exit) {
    ...
    if (pollfds[0].revents & POLLIN) {
      ...
      client_socket = accept4(listenfd, (struct sockaddr *)&client_addr, &len,
                              SOCK_CLOEXEC);
      ...
      if (client_socket <= 0) {
        ...
        // syslog(Logger::INFO, "received from %s at port %d",
