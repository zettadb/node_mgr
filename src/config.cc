...
void Configs::define_configs() {
  ...
  processes from working on the same (set of) clusters, at least the pid file
  approach deosn't work, since user can start other processes on other machines.
          We don't want to set flags on metadata server to do so because that
  could cause no cluster_mgr working at all. define_str_config("pid_file_path",
  pid_file, ".", "The directory path to store cluster_mgr's pid file to
  guarantee only a process is running.");
  */
}
...
/*
        Return the NO. of all the key variables/configs are NOT assigned a
   value. 'vars' takes back the names of such variables.
