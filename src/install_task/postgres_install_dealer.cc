...
*/
#include "postgres_install_dealer.h"
...
bool PostgresInstallDealer::fetchMetaInfoFromMetadataCluster() {
...
bool PostgresInstallDealer::constructCommand() {
  ...
  std::string install_log = kunlun::string_sprintf(
      ...
      kunlun::GetBasePath(program_binaries_path).c_str(),pg_port.c_str());
...
bool PostgresInstallDealer::Deal() {
  ...
  if (!ret) {
    ...
    PostgresExporterInstallDealer postgres_exporter(exporter_port_);
    ret = postgres_exporter.Deal();
    if(!ret) {
    ...
    Instance_info::get_instance()->add_postgres_exporter(exporter_port_);
    
    Json::Value para_json = json_root_["paras"];
    std::string pg_port = para_json["pg_protocal_port"].asString();
    Instance_info::get_instance()->add_computer_instance(data_prefix_, pg_port);
  }
  return ret;
}
void PostgresInstallDealer::AppendExtraToResponse(Json::Value &root) {
  Json::Value para_json = json_root_["paras"];
  std::string pg_port = para_json["pg_protocal_port"].asString();
  std::string bind_address = para_json["bind_address"].asString();
